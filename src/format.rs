...
use crate::{p256::PublicKey, RECIPIENT_PREFIX, STANZA_TAG};

const STANZA_KEY_LABEL: &[u8] = b"age-encryption.org/v1/piv-p256";

const TAG_BYTES: usize = 4;
const ENCRYPTED_FILE_KEY_BYTES: usize = 32;

pub(crate) fn yubikey_to_str(pk: &PublicKey) -> String {
    bech32::encode(RECIPIENT_PREFIX, pk.as_bytes().to_base32()).expect("HRP is valid")
}

pub(crate) fn yubikey_tag(pk: &PublicKey) -> [u8; TAG_BYTES] {
    let tag = Sha256::digest(yubikey_to_str(pk).as_bytes());
